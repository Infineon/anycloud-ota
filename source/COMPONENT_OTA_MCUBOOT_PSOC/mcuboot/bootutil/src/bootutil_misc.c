...
boot_write_magic(const struct flash_area *fap)
{
    ...
    if (rc != 0)
    {
//        printf("%s() FAILED TO WRITE MAGIC: 0x%lx + 0x%lx = 0x%lx (rc:%d)\n", __func__, fap->fa_off, off, (fap->fa_off + off), rc);
    }
    if (rc != 0) {
        return BOOT_EFLASH;
    }
...
boot_set_confirmed(void)
{
    ...
    if (rc == 0)
    {
        ...
        if (rc != 0)
        {
//            printf("%s() FAILED TO WRITE OK: 0x%lx + 0x%lx = 0x%lx (rc:%d)\n", __func__, fap->fa_off, off, (fap->fa_off + off), rc);
        }
        flash_area_close(fap);
    }

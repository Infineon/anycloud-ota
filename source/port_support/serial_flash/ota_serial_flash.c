...
cy_rslt_t ota_smif_initialize(void)
{
    ...
    cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "%s() call Cy_SMIF_CacheDisable()\n", __func__);
     ...
     */
    cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%s() call Cy_SMIF_Init()\n", __func__);
    smif_status = Cy_SMIF_Init(SMIF0, &ota_SMIF_config, TIMEOUT_1_MS, &ota_QSPI_context);
    if (smif_status != CY_SMIF_SUCCESS)
    {
        cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "%d Cy_SMIF_Init() failed smif_status : 0x%lx\n", __LINE__, smif_status);
        return smif_status;
    }
#endif

    /* Set up SMIF */
    cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%d call Cy_SMIF_SetDataSelect()\n", __LINE__);
    ...
    cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%d call Cy_SMIF_Memslot_Init()\n", __LINE__);
    smif_status = Cy_SMIF_Memslot_Init(SMIF0, &smifBlockConfig, &ota_QSPI_context);
    if (smif_status != CY_SMIF_SUCCESS)
    {
        cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "%d Cy_SMIF_Memslot_Init() failed smif_status : 0x%lx\n", __LINE__, smif_status);
        // CY_HALT();
        return smif_status;
    }
...
#include "flash_qspi.h"
        ...
        if (CY_SMIF_SUCCESS == qspi_status) {
            ...
            cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "External qspi_init_sfdp() Succeeded.\n");
        } else {
            result = CY_RSLT_TYPE_ERROR;
            cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "qspi_init_sfdp() FAILED: 0x%08\n",(uint32_t)qspi_status);
        }
    }
#endif

    bool QE_status = false;
    cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "%d call IsQuadEnabled()\n", __LINE__);
    smif_status = IsQuadEnabled(smifMemConfigs[0], &QE_status);
    if (smif_status != CY_RSLT_SUCCESS)
    {
        cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_WARNING, "%d IsQuadEnabled() failed smif_status : 0x%lx\n", __LINE__, smif_status);
    }
    ...
    {
        /* Enable Quad mode */
        cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_ERR, "%d call EnableQuadMode()\n", __LINE__);
        smif_status = EnableQuadMode(smifMemConfigs[0]);
        if (smif_status != CY_RSLT_SUCCESS)
        {
            // CY_HALT();
            cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_WARNING, "%d EnableQuadMode() failed smif_status : 0x%lx\n", __LINE__, smif_status);
        }
    }
...
cy_rslt_t ota_smif_erase(uint32_t offset, uint32_t length)
{
    ...
    if (IS_FLAG_SET(FLAG_HAL_INIT_DONE))
    {
        ...
        else
        {
            ...
            cy_log_msg(CYLF_MIDDLEWARE, CY_LOG_DEBUG, "Calling Cy_SMIF_MemEraseSector - offset: 0x%08lx, length: 0x%08lx\n", offset, length);

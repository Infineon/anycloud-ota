...
if [[ "$IMGTOOL_COMMAND_ARG" != "do_not_sign" ]]; then
    ...
    $CY_PYTHON_PATH $MCUBOOT_SCRIPT_FILE_DIR/$IMGTOOL_SCRIPT_NAME $IMGTOOL_COMMAND_ARG $FLASH_ERASE_ARG -e little --pad-header --align 8 -H $MCUBOOT_HEADER_SIZE -M $MCUBOOT_MAX_IMG_SECTORS -v $CY_BUILD_VERSION -L $CY_BOOT_PRIMARY_1_START -S $CY_BOOT_PRIMARY_1_SIZE $CY_SIGNING_KEY_ARG $CY_OUTPUT_HEX $CY_OUTPUT_SIGNED_HEX
fi
#
...
if [ "$CY_BUILD_SEPARATE_FW" == "FW_DATBLOCK_SEPARATE_FROM_APPLICATION" ]
then
echo "Primary 1 Slot Image 1 Start             : $CY_BOOT_PRIMARY_1_START"
echo "Primary 1 Slot Image 1 Size              : $CY_BOOT_PRIMARY_1_SIZE"
